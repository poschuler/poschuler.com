...
**What deliberately does not.** A document's own words — a Post's title, a Project's summary, the home page's biography — are content, not chrome. They belong to the document and are written by their author, so they never enter the catalogue, and no route's `meta()` reads it for `og:` copy: that would mean the catalogue inventing Spanish prose nobody reviewed. That is a decision, not an omission, and it is recorded in the catalogue's own docblock so the next reader does not close the gap by accident.

**Content can still be bilingual — beside its own page, not in the catalogue.** The home page is the one document whose text has no source outside the code, and `app/routes/home/bio.tsx` is that source: its two paragraphs, its `description` and its `knowsAbout`, as `Record<Locale, …>`, so a paragraph written in English and forgotten in Spanish fails to compile. `_home.tsx` keeps the structure and renders `HOME_BIO[locale]`; its `meta()` reads the same object, which is how a Spanish reader and a Spanish crawler are told the same thing. It is the shape `resume.json` already uses for `basics.summary`, and the Spanish there is where the home page's Spanish came from — two pages describing the same work should not describe it in two voices.

**What stays single-form inside that file, and why.** The role line, the `<title>` and `og:image:alt` are one string for both branches: they name the person and the job title, and a job title is a name for a role in a market rather than a sentence about him — translating *Senior Backend Engineer* would narrow the page rather than open it. `resume.json`'s `basics.label` made the same call. `LOCATION` in `app/lib/contact.ts` goes the other way for one word only: *Perú* carries an accent, and the footer renders it on every page, so leaving it unaccented under `/es` would be a spelling mistake repeated site-wide.
...
**A trail's upper steps come from `indexCrumb` / `indexHref`, never from a literal, in both of its emissions — the visible one above a document and the `BreadcrumbList` in its structured data.** Home, and the Index the document sits under, named with the heading that page already renders and addressed on the page's own branch — the steps below it are the document's own and the route builds those from `postHref` / `seriesHref` / `projectHref`. Writing `{ name: "Series", path: "/series" }` by hand is what made every Spanish trail the English one.

The two emissions are allowed to differ in **which steps they include**, and do: the visible one names a Series Section and omits the home page, the `BreadcrumbList` does the reverse. A Section has no page, so naming it in a `BreadcrumbList` — which describes URLs a visitor can reach — would declare a level of hierarchy that cannot be visited; above the article it is context for a human instead. They are never allowed to differ in **how a step they both have is named or addressed** — the home page and the Index a document sits under carry the same name and the same path in both emissions, because both read them from the one place, `indexCrumb` / `indexHref`.
...
| `indexCrumb(index, locale)`, `indexHref(index, locale)` | a trail's fixed upper steps — the home page, and the index a document sits under — named and addressed in the page's own Locale, for a `BreadcrumbList` step and a `<Link to>` respectively |
