...
Extracting it was not tidying. The six copies had drifted three ways: two carried a `font-family` the `<main>` above them already set, two wrapped their text where the other four did not, and two had gone to an arbitrary 10px. Only the last was ever noticed, and only because it looked wrong.
