...
| `/blog/:blogSlug`| D1 + KV | none          | the row first, Locale-aware: a Post with a container 301s to it. The KV body fetch still hardcodes `:en` (see Known defects) |
...
- **`/blog/:blogSlug`'s KV fetch hardcodes `:en`.** The D1 row lookup reads the request's Locale (Phase 3, 3/2) and would resolve a Spanish Translation's row correctly, but the KV read two lines below it still asks for `blog:${blogSlug}:en` rather than the resolved row's own `lang` — every sibling route (`/projects/:project`, `/series/:seriesSlug`, `/series/:seriesSlug/:partSlug`, `/projects/:project/:note`) already keys its KV read off the row. No URL carries a Locale yet (no `/es` branch is mounted — ADR 0010), so this is latent rather than live; fix it alongside mounting that branch.
