...
- **A `BreadcrumbList` crosses Locales half way along.** `breadcrumbList` takes no Locale and `HOME_CRUMB` is `{ name: "Home", path: "/" }`, so the fixed upper levels of every trail — *Home*, *Blog*, *Series*, *Tags* — are named in English and point at the English URL, while the levels a route builds with `seriesHref(…, locale)` or `postHref(…, locale)` correctly follow the page. A Spanish page therefore claims a path that starts on the English site and switches half way down. Two of them are reachable today, `/es/series` and `/es/tags`, because an index exists in every Locale while a Spanish document does not — so this ships as soon as the switcher is revealed, not when the first Translation lands. Closing it is a Locale on `breadcrumbList` and those four names moving into the string catalogue, which is where every other interface word already is.
