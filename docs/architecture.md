...
| `/projects/:project` | D1 + KV | none      | row frames the page, KV carries the body; both keyed off the resolved row's Locale; index of its Field Notes at the foot, via `findProjectNotes` |
...
**The model layer takes the Locale as a required, typed argument**, not a default parameter — a deliberate compile break across every call site when it landed, which is what turned *a query forgot the Locale* from a runtime defect into one the compiler finds. Two queries are the deliberate exception: the Bookmark query and the Timeline query filter with `lang = ? or lang is null` rather than plain equality, because a Bookmark has no Locale and belongs to both — the glossary's definition of the Timeline is what forces it (`CONTEXT.md`).

**One module answers what address a document has.** `app/lib/hrefs.ts` stays the only place a *relative* path is built — `withLocale` and the per-kind helpers (`postHref`, `projectHref`, `seriesHref`) all funnel through `ES_PREFIX`, so the two branches cannot disagree about what marks the Spanish one. `app/lib/seo/alternates.ts` is the one place a path is made absolute and given its siblings: `documentAddresses` takes a document's identity, its own Locale and the set of Locales that exist for it, and returns the canonical, the reciprocal `hreflang` alternates and the `x-default` (the English address). The page `<head>`, the structured data and the sitemap all read it — nothing reconstructs the rule a second time, which is what makes reciprocity a property of the system rather than of whoever last edited a route. The same module's `switcherDestination` computes where the language switcher sends a reader — the counterpart document when a Translation exists, the section index when it does not — from the identical `existingLocales` a route's own `documentAddresses` call already computed, so the switcher and the `hreflang` set cannot drift apart.
...
- **`findAllProjects` has no Locale filter.** Every other listing query gained a required, typed Locale in Phase 3's 3/2, but `project.server.ts`'s `findAllProjects` — read by `/` and `/projects` — still returns every Project regardless of Locale. Harmless while every Project is English; the day a Spanish Project landing is seeded, both pages list the same Project twice, once per Locale.
