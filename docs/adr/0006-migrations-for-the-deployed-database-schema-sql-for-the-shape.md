...
## Amendment: a column is dropped a deploy later than its last reader
...
## Amendment: a rename is an expand-and-contract too, and its second step is gated on a deploy

The amendment above is written as a *drop*, and the recurrence it predicted arrived as a **rename**, which looks like neither an add nor a drop until you notice that `ALTER TABLE ... RENAME COLUMN` is one atomic statement. That is exactly what makes it unsafe here: it removes the old name during the migration step, while the Worker still serving asks for it. So a rename is not one move but three — add the new column, move the reader, drop the old one — and the same two-publication boundary falls in the same place.

`content.section_order` → `content.container_order` is the worked instance, and it went through both steps. The first publication added `container_order` in `0006`, left `section_order` in place written by the generator with the same value, and shipped the Worker reading the new column *in the same deploy*; the next publication dropped the old one in `0007`. Two publications rather than the three `content.tags` took, and the difference is only where the reader moves: `tags` was still being read in its expand step, so it needed a publication of its own to stop. A column that switches its reader in the expand step costs two; one that does not costs three.

**The gate is a deployment, not a merge.** This is the part the drop case never had to state, because the two were the same in practice. They are not: the publication job applies migrations, verifies the schema, seeds both stores, verifies them, and only then builds and deploys the Worker. For those first five steps the Worker answering production is still the previous one. A contract migration merged into `main` alongside its expand half would therefore fail in exactly the way the amendment above describes, and a contract migration merged while the expand half is green in CI but not yet deployed would fail the same way. So the second step waits on the deploy being confirmed live, which is what the ticket that carried `0007` was blocked on.
