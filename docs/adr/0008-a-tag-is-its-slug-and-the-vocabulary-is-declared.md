...
- **A Tag page lists Posts and never Bookmarks**, which is what keeps `CONTEXT.md`'s Timeline entry true: the Timeline is *the only place the two kinds appear together*, strictly enough that the same entry insists the home page excerpt is not a Timeline. That sentence was checked against this decision rather than left to survive by luck, and it needed no rewriting. The rejected middle option — Posts anchor the page, Bookmarks in their own section below — stays available without a migration, because the table holds rows for both kinds.
