...
- Row types (`ContentRowType`) are hand-maintained and **not** checked against the schema by anything. A column rename breaks at runtime, not at `tsc`. They are shaped after the domain rather than the table: a discriminated union on `type`, so the columns a Post or a Bookmark does not carry are typed `null`.
