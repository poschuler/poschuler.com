/**
 * **No `/es/cv.pdf` (see #48's open question, answered).** `/cv` renders from
 * `resume.json` at request time, so once that document carried Spanish text
 * (#48) the Spanish page followed with no extra work. This route does the
 * opposite: it proxies one hand-produced static file from a CDN — nothing here
 * renders a PDF from `resume.json` — so a Spanish download would be a second
 * file, authored and uploaded by hand, kept in step with the English one
 * forever. That is the same failure mode that ruled out a `resume.es.json` —
 * one document carrying per-Locale text, never two documents to keep in step —
 * transposed to a binary this codebase cannot generate. `/cv`'s own download
 * button (`routes/resume/hero.tsx`) therefore still points at this one URL from
