...
const SelectTrigger = ({ className, children, ...props }: AriaButtonProps) => (
  <AriaButton
    className={composeRenderProps(className, (className) =>
      cn(
        "flex h-10 w-full items-center justify-between rounded-md border border-input bg-subtle px-3 py-2 text-sm ring-offset-background",
        /* Disabled */
...
const SelectPopover = ({ className, ...props }: AriaPopoverProps) => (
  <Popover
    className={composeRenderProps(className, (className) =>
      cn("w-[--trigger-width] border border-default", className)
    )}
    {...props}
  />
);
...
function CustomRowSelect<T extends object>({
  label,
  description,
  errorMessage,
  children,
  ...
  return (
    <Select
      className={composeRenderProps(className, (className) =>
        cn("group flex gap-2 items-center")
      )}
      {...props}
    >
      <Label className="text-nowrap">{label}</Label>
      <SelectTrigger className={cn(className, "border border-default")}>
        <SelectValue />
      </SelectTrigger>
      {description && (
        <Text className="text-sm text-muted-foreground" slot="description">
